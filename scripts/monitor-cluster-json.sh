...
# Usage: ./monitor-cluster-json.sh <namespace> <cluster-name>
...
if [[ -z "$NAMESPACE" ]] || [[ -z "$CLUSTER_NAME" ]]; then
    echo '{"error": "Usage: '"$0"' <namespace> <cluster-name>"}' | jq .
    exit 1
fi

# Get Cluster
CLUSTER_JSON=$(kubectl get cluster "$CLUSTER_NAME" -n "$NAMESPACE" -o json 2>/dev/null || echo '{}')
...
if [[ -n "$INFRA_KIND" ]] && [[ -n "$INFRA_NAME" ]]; then
    ...
    INFRA_JSON=$(kubectl get "$INFRA_RESOURCE" "$INFRA_NAME" -n "$NAMESPACE" -o json 2>/dev/null || echo '{}')
...
if [[ -n "$CP_KIND" ]] && [[ -n "$CP_NAME" ]]; then
    ...
    CP_JSON=$(kubectl get "$CP_RESOURCE" "$CP_NAME" -n "$NAMESPACE" -o json 2>/dev/null || echo '{}')
...
MP_JSON=$(kubectl get machinepool -n "$NAMESPACE" -l cluster.x-k8s.io/cluster-name="$CLUSTER_NAME" -o json 2>/dev/null || echo '{"items":[]}')
...
if [[ "$MP_COUNT" -gt 0 ]]; then
    ...
    while IFS= read -r mp; do
        ...
        if [[ -n "$INFRA_MP_KIND" ]] && [[ -n "$INFRA_MP_NAME" ]]; then
            ...
            INFRA_MP_JSON=$(kubectl get "$INFRA_MP_RESOURCE" "$INFRA_MP_NAME" -n "$NAMESPACE" -o json 2>/dev/null || echo '{}')
...
if kubectl get secret "$KUBECONFIG_SECRET" -n "$NAMESPACE" &>/dev/null; then
    NODES_JSON=$(kubectl get secret "$KUBECONFIG_SECRET" -n "$NAMESPACE" -o jsonpath='{.data.value}' | base64 -d | \
        KUBECONFIG=/dev/stdin kubectl get nodes -o json 2>/dev/null || echo '{"items":[]}')
